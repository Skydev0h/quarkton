...
* Finish implementation of TON Connect 2.0. For now many hurdles were encountered during the implementation. First of all, wallet not in wallets list cannot connect. Therefore for development I had to temporarily supply Tonkeeper as wallet name. Most importantly, for some unknown reason App immediately disconnects after connection. I can guarantee that encryption is correct since app receives, decrypts message and right away sends disconnect message without explaining why in any way, either in message, or in console, or with some popup message. I was not able to deal with that problem and that prevented from properly implementing TON Connect until the deadline. Actually the required UI for TON Connect is implemented and additional UI for working with bridges and Apps is designed and planned, but without being able to actually test on a live App I was not able to continue this deveopment further.
